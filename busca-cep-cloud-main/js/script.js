...
// Função para limpar os campos da aba de Busca por Rua
function limparCamposRua() {
  // Redefinir o seletor de Estado (UF)
  const seletorUf = document.getElementById('uf-rua');
  seletorUf.value = '';
  M.FormSelect.init(seletorUf);

  // Redefinir o seletor de Cidade
  const seletorCidade = document.getElementById('cidade-rua');
  seletorCidade.innerHTML = `<option value='' disabled selected>Selecione a Cidade</option>`;
  M.FormSelect.init(seletorCidade);

  // Limpar o campo de texto do Logradouro (Rua)
  document.getElementById('rua').value = '';

  // Limpar a lista de resultados de endereços
  document.getElementById('lista-ruas').innerHTML = '';

  // Atualizar os rótulos (labels) dos campos de texto do Materialize
  M.updateTextFields();
  
  registrarLog('Ação: Os campos da busca por rua foram limpos.');
}